...
> **⚠ HISTORICAL AS OF 2026-09-22 (LANE-CONTINUITY, `docs/DECISIONS.md` rows 64–65).**
> There is no `WEBGL_ZOOM_CAP` any more. The cap was lifted 1e4 → 1e6 by GPU-ARBITRARY
> and then **REMOVED** by LANE-CONTINUITY (owner directive row 62: no hard stop). What
> this section describes is the *plain float32 absolute-coordinate* path that the cap
> originally guarded; the shipped GPU path is the perturbation lane, whose own limits
> are RANGE (fixed) and MANTISSA (the current single-factor solver, measured correct to
> 1e-40), and the only depth signal in the UI is an informational notice.
...
| **P3** | **The path switch** — plain float32 ↔ perturbation — placed where the plain path's error is already sub-pixel, with no visible change across the boundary. **LANDED 2026-09-22 (LANE-CONTINUITY).** The switch itself was measured to be INVISIBLE and left where it is; the visible 31.6-unit jump at 1e-4 was D2's budget floor turning on in the same wheel step, and that floor is now introduced continuously. | a zoom sweep across the switch shows no spike; at the switch the two paths agree within the stated threshold — **both measured**: with the effective budget held FIXED (slider 4096/8192) the lane switch moves the frame by **0.013** mean-red units at the boundary, against 31.600 for the removed budget rule, and the shipped sweep's boundary step is **0.019** against the removed rule's 31.600 (baseline in-pin). Pins `tests/lane-continuity.spec.js` 1–3; decisions rows 64–65. | **LANDED & VERIFIED** |
| **P4** | **Retire the CPU handoff** over the range the GPU now covers, and lift the cap past the range wall. **LANDED 2026-09-21 (`session-gpu-arbitrary`, branch `feat/gpu-arbitrary`).** The float32 delta-coordinate RANGE wall is removed by a uniform SPLIT (`u_scale <- scale * 2^shift`, `u_scaleShift <- shift`, `S = exp2(-u_scaleShift)`) — the delta is seeded in float32's NORMAL range and the exponent rides the rescaled representation's own `S`, with the physical offset unchanged. The automatic CPU switch is REMOVED (`#zoomCapOffer` and its buttons deleted; the cap now reports a brief non-modal notice) and the manual checkbox plus the genuine no-GPU path are untouched. | beyond the old cap the GPU stays selected and correct; the offer does not appear in the covered range; CPU stays reachable — **all three measured.** Through the REAL render path at 128x96 against an independent per-pixel BigInt iteration: **1e-40 renders at 0.00000 % misclassified, mean \|Δ escape index\| 0.003, worst 2**, where the SAME view through the pre-fix seed is **100 % misclassified with ONE distinct value** (baseline in-pin, `tests/gpu-arbitrary.spec.js` pin 1). The old wall (2e-38) is no longer special (pin 3). `WEBGL_ZOOM_CAP` is 1e4 -> **1e6** here, set from the measured cost curve (14 ms/image at 1e-4 to 640 ms/image at 1e-40 at 640x480 on this host's SOFTWARE rasteriser) rather than from precision (pin 3, `docs/DECISIONS.md` row 49). **THE CAP WAS THEN REMOVED ENTIRELY by LANE-CONTINUITY (2026-09-22, rows 64–65): `WEBGL_ZOOM_CAP = null`, depth is bounded by TIME, and the reach is reported non-modally instead of enforced.** The per-full-image render time is user-visible and observable (pin 2, row 50). **NO REGRESSION proven byte-for-byte: the shallow plain lane's readback hashes are IDENTICAL to the pristine pre-change build at four shallow views** (pin 4, row 52a), and P1's 1e-15 pins stay green. Recorded limit of the CURRENT single-factor solver: it degrades past ~1e-41 (7.4 % at 1e-42) — **a MANTISSA limit of the solver we built, not a numerical floor: the fix is a wider mantissa (compensated / multi-component delta), which the precision probe already measured at 10.4–12.5× fragment cost and deliberately did not build** — and the two-factor alternative measured WORSE (32 % at 1e-40), so the shipped reach is measured, not asserted (row 48; rows 64–65 name the three separate limits). Also recorded: the deep lane's boundary at 1e-4 is a PROGRAM SWITCH, whose own colour delta is **0.013** at a FIXED budget (measured, `tests/lane-continuity.spec.js` pin 2) — the 0.469 hop fraction the writer recorded was a sub-iteration proxy that overstated it; the visible jump was the budget floor, now continuous. | LANDED 2026-09-21, `session-gpu-arbitrary`, branch `feat/gpu-arbitrary`; cap removed 2026-09-22 by `session-lane-continuity` (rows 64–65) |
...
| **D7** | **GPU coarse-to-fine refinement, and a render that never queues a zoom behind it** (owner request, 2026-09-22: *"They get slow of course and thats where the coarse to fine rendering should come in. Rendering that does not block further zoom. Can do that for any depth, it just wont be noticable at lower depths."*). A render becomes a CHAIN of levels `8 -> 4 -> 2 -> 1` — the CPU worker's own sequence — each level a REAL pass of the SAME fragment program into a `ceil(W/step) x ceil(H/step)` offscreen target magnified on present, so a level costs `1/step^2` of the final pass and only the SAMPLE DENSITY changes (the arithmetic still exists once, row 52 of DECISIONS). The chain yields a macrotask between levels and checks its generation token FIRST, so a wheel arriving mid-render abandons the remaining levels at once and its own first visible response is the cheapest level. The final level is byte-for-byte the pre-refinement frame. | a deep render applies MORE THAN ONE frame with genuinely coarser-then-finer content, each level measurably cheaper than the final one; a view change during refinement is honoured at the next level boundary rather than after the chain (with the pre-change single-pass schedule as the FAILING BASELINE in-pin); and the FINAL frame is byte-identical to the single-pass build's at deep and shallow views — **all measured**: levels `8:4.5 4:5.7 2:15.5 1:58.2 ms` at scale 1e-15/64x48 (SwiftShader), equal-neighbour fraction `0.902>0.806>0.656>0.403`, 8x8-uniform-block fraction `1.000>0.083>0.042>0.021`, wheel-to-first-visible-frame `3.8 ms` against the `[1]` schedule's `30.2 ms`, superseded chain applies 1 level and SKIPS 3, and the `[1]` frame equals the chain's final frame byte for byte. Pins `tests/gpu-refine.spec.js` 1–4. | **LANDED & VERIFIED** at `1a80ed8` (branch `feat/refine`, writer `session-…-refine`); decisions rows 84–86. |
...
| **P5** | **Lift the zoom cap** so any of this becomes user-visible; retire the CPU handoff over the covered range. **LANDED 2026-09-21 (`session-gpu-arbitrary`, cap 1e4 → 1e6) and COMPLETED 2026-09-22 (`session-lane-continuity`): the cap is REMOVED entirely — `WEBGL_ZOOM_CAP = null`.** Depth is bounded by time and reported, never stopped; the ONE remaining depth signal is the informational measured-correct-reach notice. | beyond the old cap the GPU stays selected and correct; the offer no longer appears in the covered range — measured, plus (2026-09-22) a scale of 1e-45 stored verbatim through the wheel, `setView` and the render path, and the 1e-4 lane boundary's colour step reduced from 31.600 to 0.019 (baseline in-pin) |
...
**SLICE THAT WENT FIRST — COARSE-TO-FINE REFINEMENT (D7, LANDED 2026-09-22 at `1a80ed8`).**
It edits the same shader the mantissa ladder does and was dispatched ahead of it because it is
the owner's live responsiveness request; it touches the PASS SCHEDULE around the fragment
program, not the delta arithmetic, so the ladder's work is unchanged by it — but the two must
still not be in flight together.

**NEXT SLICE — the MANTISSA LADDER (named 2026-09-22, NOT built: it edits the same shader
`session-lane-continuity` was in, so it is SERIALIZED after it).** The shipped single-factor
float32 delta is limited by its **24-bit mantissa** (measured correct to 1e-40, 7.4 %
misclassified at 1e-42); the exponent split fixed RANGE, and widening the mantissa is the
next step — compensated / multi-component delta, **k components ≈ k·24 bits at ≈ k× cost**.
The committed precision probe already measured the 2-component case (hi+lo holds to ≥1e10 at
0.000 % misclassification, `docs/PROBE-2026-09-21-gpu-precision.md` §8; `docs/STATE.md`
MEASURED-FINDINGS) and the fork deliberately did not build it. **The exponent split composes
cleanly with it**: mantissa components are independent of the exponent, and the rescaled
representation scales every component by the same exact power of two `S` (`2^shift * 2^-shift
=== 1`), so a multi-component `dcx/dcy` needs the same `S` applied to each component — no
change to the seed, the shift rule or the ramp. The cost is linear in the component count, so
depth becomes slower, never stopped (owner directive row 62).
